...
从 $(0, 0)$ 走到 $(a, 0)$，规定每次只能走到左下或者右下，且必须有**恰好一次**传送（向下 $b$ 单位），且不能走到 $x$ 轴下方，方案数为 $\dbinom{a+1}{\frac{a-b}{2}+k+1}$ 。TODO：原式含未定义的 $k$，与组合数学章同一条，保留待核。
...
从 $(0, 0)$ 走到 $(a, 0)$,规定每次只能走到左下或者右下,且必须有**恰好一次**传送(向下 $b$ 单位),且不能走到 $x$ 轴下方,方案数为 $\dbinom{a+1}{\frac{a-b}{2}+k+1}$ 。TODO：原式含未定义的 $k$，无法复原，保留待核。
