...
可以直接使用凸包模板，但是代码较长；在这里我们使用暴力枚举试点，也能以 $\mathcal O(N)$ 的复杂度通过。当两个向量的叉乘 $\le0$ 时说明其夹角大于等于 $180^\circ$ ，使用这一点即可判定。
...
多模式串同时在文本里匹配。先 `insert` 每个模式，`build()` 求 fail（失配跳到当前后缀里最长的已有前缀），再 `query(文本)`。fail 把 Trie 连成 KMP 自动机，匹配沿边走即可。第二份 `add` 返回模式终点，`work(文本)` 在 fail 树上汇总出现次数。复杂度 $\mathcal O(\sum|s_i|+|S|)$，字符集默认 $26$。
...
识别一个串的全部子串：每个状态对应 endpos 相同的一类子串。`len` 是该状态最长串的长度，`link` 指向更短的后缀状态。逐字符 `last = extend(last, c)`（第一份从节点 `p` 接字符 `c`，返回新 last；多串时 last 复位为 $0$ 即广义 SAM）。本质不同子串数 $\sum(\mathrm{len}[v]-\mathrm{len}[\mathrm{link}[v]])$，复杂度 $\mathcal O(N\log|\Sigma|)$。
...
最基础的取石子：每次 $1\sim M$ 颗，判胜负只看 $N$ 对 $M+1$ 取模。
...
**存在某一位，** $\pmb{One_i}$ **不是** $\pmb{K+1}$ **的倍数。**（原来写成“每一位均不为倍数”，过严，应是存在一位。）

### Anti-Nim 游戏 (反 Nim 游戏)

最后取者输。全堆大小均 $\le 1$ 时看堆数奇偶（即异或是否为 $0$）；否则与普通 Nim 同判据。
...
- 对于非最终态的某个节点，其 SG 值为所有子节点的 SG 值取 $\mathrm{mex}$ ；
...
结论：当根节点 SG 值非 $0$ 时先手必胜。（原文写“非 $1$”，与上面叶子 SG $=0$ 的约定不一致；板子比较的是返回值 $R=\mathrm{SG}+1$ 是否为 $1$。）
...
使用结构体存边（该算法无需存图），以 $\mathcal{O} (NM)$ 的复杂度计算，注意，当所求点的路径上存在负环时，所求点的答案无法得到，但是会比 INF 小（因为负环之后到所求点之间的边权会将 `d[end]` 的值更新），该性质可以用于判断路径上是否存在负环：在 $N-1$ 轮后仍无法得到答案（一般与 `INF / 2` 进行比较）的点，到达其的路径上存在负环。
...
枚举中转点 $k$，用 $i\to k\to j$ 松弛；$k$ 必须最外层。可负权，有负环则对角线会被更新成负。邻接矩阵、下标 $1..n$，无边先赋无穷。这里加的是单向边，双向要加两次。复杂度 $\mathcal{O}(N^3)$。
...
平面图最小割可以转化为对偶图上的最短路：把原图的每个面当作对偶图的点，跨越某条边的两个面之间连一条权值为该边容量的边，原图 $s\to t$ 的最小割即对偶图的最短路（典型题：狼抓兔子）。下方代码把格点通过 `Hash` 映射成对偶图节点编号。
...
> 性质：缩点后的图拥有拓扑序 $color_{cnt}, color_{cnt-1},…,1$ ，可以不需再另跑一遍 `topsort`；缩点后的图是一张有向无环图（DAG、拓扑图）。
...
数组模拟邻接表（链式前向星）：比 `vector` 邻接表常数更小、代码稍长，需要大量加边或卡常时常用。`dfs`：标准复杂度为 $\mathcal O(N+M)$。节点子节点的数量包含它自己（至少为 $1$），深度从 $0$ 开始（根节点深度为 $0$）。`bfs`：深度从 $1$ 开始（根节点深度为 $1$）。`topsort`：有向无环图（包括非联通）才拥有完整的拓扑序列（故该算法也可用于判断图中是否存在环）。每次找到入度为 $0$ 的点并将其放入待查找队列。
...
与二分图匹配的差别在于图中可能存在奇环：算法在找增广路时把奇环缩成"花"（blossom）统一处理，时间复杂度与边的数量无关、与点数有关，为 $\mathcal O(N^3)$ 。下方模板编号从 $0$ 开始，`work()` 返回 `{最大匹配数, match 数组}`，其中 `match[i]` 为 $i$ 的匹配点，$-1$ 表示未匹配；例题为 [UOJ #79. 一般图最大匹配](https://uoj.ac/problem/79) 。
...
下方模板编号从 $1$ 开始，复杂度为 $\mathcal O(N^3)$ 。调用 `work(n, edges)` 返回最大总权（`edges` 元素为 `{u, v, w}`，重边自动取权值最大者）；`match()` 返回其中一种方案的配对表（每点 $i$ 配 `lk[i]`，一个匹配边可能出现两次）。权值类型由 `typedef int T` 决定，需要时改为 `long long`。
...
该算法基于最大流，常数极小，且引入随机化，几乎卡不掉。最坏时间复杂度为 $\mathcal O(\sqrt NM)$ ，经[测试](https://judge.yosupo.jp/problem/bipartitematching)，在 $N,M$ 均为 $2 \times 10^5$ 的情况下能在 60ms 内跑完。
...
使用 KM（Kuhn–Munkres）算法解（与上一节的匈牙利算法常被混称），时间复杂度为 $\mathcal O(N^3)$ ，要求存在完美匹配。下方模板用于求解最大权值、且可以输出其中一种可行方案：`work()` 返回最大权，`getMatch(n1, n2)` 输出方案；例题为 [UOJ #80. 二分图最大权匹配](https://uoj.ac/problem/80) 。
...
DAG 上按拓扑序松弛：入度 $0$ 入队，`dis[y]=\max(\mathrm{dis}[y],\mathrm{dis}[x]+w)`。有环先 Tarjan 缩点。`topsort(s,t)` 返回 $s$ 到 $t$，不可达为 $-10^{18}$。复杂度 $\mathcal O(N+M)$。
...
要求每个节点的出边按目标编号**升序**排列（读入后排序，或直接存进 `multiset`），每次取最小出边即保证字典序最小；起点 `s` 按上文判定选取（欧拉回路可任取非孤立点）。注意下方用 `vector::erase(begin())` 每次开头删除是 $\mathcal O(\deg)$，总复杂度退化为 $\mathcal O(M^2)$，边多时改用 `multiset` 或指针法。
...
$$x_{u_i} - x_{v_i} \leq w_i \quad (i = 1, 2, \dots, m)$$

求任意一组解。做法：把 $x_u - x_v \leq w$ 写成 $x_u \leq x_v + w$，从 $v$ 向 $u$ 连一条权值为 $w$ 的边跑最短路，最短路数组本身即为一组解；存在负环则无解。下方代码实际是 Bellman–Ford（每轮松弛全部 $m$ 条边、共 $n-1$ 轮，复杂度 $\mathcal O(nm)$，原文标注 SPFA 系误称），存边时 `e[i] = {v, u, w}` 正好实现 $v \to u$ 这条边。**注意这里以 `d[1] = 0` 作单源，图不连通或存在从 1 不可达的点时需改为超级源点：向所有点连权 0 的边（即初始化全部 `d[i] = 0`）。**[参考](https://www.luogu.com.cn/problem/P5960)
...
3. 给出一棵树，要求添加尽可能多的边，使得其是二分图：对树进行二分染色，显然，相同颜色的点之间连边不会破坏二分图的性质，故可添加的最多的边数即为 $cnt_{\mathrm{Black}}\cdot cnt_{\mathrm{White}}-(n-1)$ ；
...
将原图的 SCC 缩点，统计缩点后的新图上入度为 $0$ 和出度为 $0$ 的点的数量 $cnt_{\mathrm{in}}$、$cnt_{\mathrm{out}}$，答案即为 $\max(cnt_{\mathrm{in}}, cnt_{\mathrm{out}})$ 。过程大致是先将一个出度为 $0$ 的点和一个入度为 $0$ 的点相连，剩下的点随便连 [See](https://www.acwing.com/problem/content/369/) 。

---

题意：添加最少的边，使得无向图变成一个 E-DCC

将原图的 E-DCC 缩点，统计缩点后的新图上度为 $1$ 的点（叶子结点）的数量 $cnt$ ，答案即为 $\left \lceil \frac{cnt}{2} \right \rceil$ 。过程大致是每次找两个叶子结点（但是还有一些条件限制）相连，若最后余下一个点随便连 [See](https://www.acwing.com/problem/content/397/) 。
...
依赖链式前向星段的 `h / tot / ver / ne / edge / add`；`Z` 为取模类（见杂项章），`INF` 需自行定义。下方 `Solve` 把边权固定为 $1$（`w = 1`），统计起点到每个点的**最短路条数，以及长度恰好为最短路 $+1$ 的次短路条数**，按题目修改边权读入即可。
...
计算两个凸包的向量和 $\{p+q\}$，结果仍是凸包。做法：各自从最低点起把边向量按极角归并依次相接。输入须已是有序凸包。复杂度 $\mathcal O(n+m)$。
...
零散题型。`LL` 为 `long long`，`N` / `BIT` 按题目或见数据结构章。

### 逆序对 (归并排序解)

$\mathcal O(N\log N)$。数组 $1$-index。`N` 按题目改。
...
从 $N$ 个整数中任选 $K$ 个相加，枚举组合。$\mathcal O(\binom{N}{K}\cdot K)$。
...
- $f(a,b)=\displaystyle\binom{a}{\tfrac{a+b}{2}}$ ；
...
从 $(0, 0)$ 走到 $(a, 0)$，规定每次只能走到左下或者右下，且必须有**恰好一次**传送（向下 $b$ 单位），且不能走到 $x$ 轴下方，方案数为 $\displaystyle\binom{a+1}{\frac{a-b}{2}+k+1}$ 。TODO：原式含未定义的 $k$，与组合数学章同一条，保留待核。
...
构造字典序最小填充，不用搜。宫 $2^N\times 2^M$，整盘 $(2^{N+M})^2$。公式 $0$-index。
...
$\mathcal O(N\log N)$ 建树与单点修改（每步新建一条链），查询 $\mathcal O(\log N)$。原理：每次修改只新建根到目标叶子的一条链（$\log$ 个节点），其余子树与旧版本共享——`root[i]` 因此就是「前 $i$ 个元素」的权值线段树，区间 $[l,r]$ 的信息由两版本相减得到（`kth` 里的 `cnt` 相减即此）。用法：把值域离散化到 $[1, m]$，`modify(root[i] = root[i - 1], ..., 离散值)` 建版本；静态区间第 $k$ 小查询 `kth(root[r], root[l - 1], 1, m, k)`。数组开 `4 * N + 17 * N`（约 17 层/次修改），`N` 按题目改。
...
与欧几里得同一递归，回溯时 $x'=y,\; y'=x-(a/b)y$ 还原系数。返回 $\gcd$；$ax+by=c$ 有解当且仅当 $\gcd\mid c$，通解 $x\pm b/d$、$y\mp a/d$。
...
以 $\mathcal O(\sqrt {P})$ 的复杂度求解 $a^x \equiv b(\bmod P)$ 。其中标准 BSGS 算法不能计算 $a$ 与 $MOD$ 互质的情况，而 exbsgs 则可以。
...
`Zmod<MOD>` 把值包成自动保持在 $[0,p)$ 的模整数类型，四则与快速幂直接对对象写，不用每步手动 `%`。多项式、组合数学章的 `mint`/`Z` 即此类。附编译期 `mypow`。
...
`fread` 一次性读 $2^{21}$ 字节进缓冲区再逐个解析，比关同步的 `cin` 还快，$10^6$ 级以上输入用。注意读入到文件结尾才结束，直接运行会无输出。
...
对拍器：Linux 做成函数追加到 `~/.bashrc`；Windows 存成 `stress.py`。
...
- 二项式反演：$\displaystyle f_n=\sum_{i=0}^n\binom{n}{i}g_i\Leftrightarrow g_n=\sum_{i=0}^n(-1)^{n-i}\binom{n}{i}f_i$，$\displaystyle f_k=\sum_{i=k}^n\binom{i}{k}g_i\Leftrightarrow g_k=\sum_{i=k}^n(-1)^{i-k}\binom{i}{k}f_i$ ；
- $\displaystyle \sum_{i=1}^{n}i{n\choose i}=n * 2^{n-1}$ ；
- $\displaystyle \sum_{i=1}^{n}i^2{n\choose i}=n*(n+1)*2^{n-2}$ ；
- $\displaystyle \sum_{i=1}^{n}\dfrac{1}{i}{n\choose i}=\sum_{i=1}^{n}\dfrac{2^i-1}{i}$ ；
- $\displaystyle \sum_{i=0}^{n}{n\choose i}^2={2n\choose n}$ ；
- 拉格朗日恒等式：$\displaystyle \sum_{i<j}(a_ib_j-a_jb_i)^2=(\sum_{i}a_i^2)(\sum_{i}b_i^2)-(\sum_{i}a_ib_i)^2$ 。
...
或 第二类斯特林数乘上 $m$ 的阶乘 $m!\cdot\mathrm{Stirling2}(n,m)$ ，答案为 $dp[n][m] * m!$ 。
...
考虑使用 Burnside 引理。红色、蓝色珠子各两个，共计有 $\displaystyle\binom{4}{2}=6$ 种染色方案。空间对称群 $G=\{r_0,r_1,r_2,r_3\}$ 分别对应旋转 $0\sim3$ 次，则它们对应的不动点集合分析如下：
...
排列与其字典序排名互转。普通解 $\mathcal O(n^2)$，树状数组解 $\mathcal O(n\log n)$，答案常取模。
...
**定理 3.1:** $n$ 阶不交 Dyck 路对数为（$C_k$ 为卡特兰数）:
$$ C_{n+2}C_n - C_{n+1}^2 $$
...
从 $(0, 0)$ 走到 $(a, 0)$,规定每次只能走到左下或者右下,且必须有**恰好一次**传送(向下 $b$ 单位),且不能走到 $x$ 轴下方,方案数为 $\displaystyle\binom{a+1}{\frac{a-b}{2}+k+1}$ 。TODO：原式含未定义的 $k$，无法复原，保留待核。
...
4. 在$n$个红球中插入$m$个蓝球,空格大小不限,不可插入两边,方案数为 $C(n+m-2,\ n-2)$
   证明:考虑有$n-1$个空格可以插入,那么问题转化为不定方程 $X_1+\dots+X_{n-1}=m, (X_i\geq 0)$ 的解的个数，即 $C(m+n-2,\ n-2)$（原来误写成 $C(n+m-2,m-2)$）
