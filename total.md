...
题目录下放三个源文件：`gen.cpp`（数据）、`x.cpp`（被测）、`xstd.cpp`（暴力）。
对拍器做成函数，追加到 `~/.bashrc` / `$PROFILE`，在该目录执行 `chk`。
编译产物与 `test.in` / `x.out` / `xstd.out` 均放在临时目录固定路径，下次覆盖，不 `rm`。
挂了去看临时目录里的输入输出。三个 `g++` 都成功才进入循环。

Linux（`~/.bashrc` 末尾）：

```bash
# 当前目录：gen.cpp x.cpp xstd.cpp
# /tmp/gen /tmp/x /tmp/xstd /tmp/test.in /tmp/x.out /tmp/xstd.out
chk() {
    g++ -std=gnu++20 -O2 -pipe -o /tmp/gen gen.cpp &&
    g++ -std=gnu++20 -O2 -pipe -o /tmp/x x.cpp &&
    g++ -std=gnu++20 -O2 -pipe -o /tmp/xstd xstd.cpp && {
        t=1
        while :; do
            echo $t
            /tmp/gen > /tmp/test.in
...
# 当前目录：gen.cpp x.cpp xstd.cpp
function chk {
    g++ -std=gnu++20 -O2 -pipe -o "$env:TEMP\gen.exe" gen.cpp
    if (-not $?) { return }
    g++ -std=gnu++20 -O2 -pipe -o "$env:TEMP\x.exe" x.cpp
    if (-not $?) { return }
    g++ -std=gnu++20 -O2 -pipe -o "$env:TEMP\xstd.exe" xstd.cpp
    if (-not $?) { return }
    $t = 1
    while ($true) {
        Write-Host $t
