...
4. **Extracted code must stay compilable.** Files under `code/` are checked by `g++ -std=gnu++20 -fsyntax-only` in check.sh. Standalone syntax checking does not establish that the printed region compiles. Regression tests extract the actual printed blocks, supply only documented dependencies, instantiate their interfaces, and run the complete usage examples. Inline code needs the same checks. Synchronize registered editor snippets with `uv run python tools/sync_snippets.py --write`.
