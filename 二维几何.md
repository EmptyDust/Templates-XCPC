...
template<typename T, typename S> bool equal(T x, S y) {
...
```c++
template<typename T> struct Point { // 在C++17下使用 emplace_back 绑定可能会导致CE！
    T x, y;
    Point(T x_ = 0, T y_ = 0) : x(x_), y(y_) {} // 初始化
    template<typename U> operator Point<U>() { // 自动类型匹配
...
template<typename T> struct Line {
    ...
    template<typename U> operator Line<U>() { // 自动类型匹配
...
```c++
template<typename T> T cross(Point<T> a, Point<T> b) { // 叉乘
    return a.x * b.y - a.y * b.x;
}
template<typename T> T cross(Point<T> p1, Point<T> p2, Point<T> p0) { // 叉乘 (p1 - p0) x (p2 - p0);
...
```c++
template<typename T> T dot(Point<T> a, Point<T> b) { // 点乘
    return a.x * b.x + a.y * b.y;
}
template<typename T> T dot(Point<T> p1, Point<T> p2, Point<T> p0) { // 点乘 (p1 - p0) * (p2 - p0);
