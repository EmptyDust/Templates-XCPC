...
堆优化 $cal(O) (M log N)$。每次弹出当前距离最小的未确定点，用它松弛邻边；正权保证弹出即为最终答案。`d` 初值 INF。

=== （负权图）Bellman ford 算法
<负权图bellman-ford-算法>
使用结构体存边（该算法无需存图），以 $cal(O) (N M)$ 的复杂度计算，注意，当所求点的路径上存在负环时，所求点的答案无法得到，但是会比 INF 小（因为负环之后到所求点之间的边权会将 `d[end]` 的值更新），该性质可以用于判断路径上是否存在负环：在 $N - 1$ 轮后仍无法得到答案（一般与 `INF / 2` 进行比较）的点，到达其的路径上存在负环。
...
=== （负权图）SPFA 算法
<负权图spfa-算法>
以 $cal(O) (K M)$ 的复杂度计算，最坏 $cal(O) (N M)$，且可以被特殊构造卡掉，无负权时优先用堆优化 Dijkstra。

#include-code("code/图论/负权图SPFA-算法.cpp")

== 多源汇最短路 \(Floyd)
<多源汇最短路-floyd>
枚举中转点 $k$，用 $i arrow.r k arrow.r j$ 松弛；$k$ 必须最外层。可负权，有负环则对角线会被更新成负。邻接矩阵、下标 $1 . . n$，无边先赋无穷。这里加的是单向边，双向要加两次。复杂度 $cal(O) (N^3)$。
...
=== （稠密图）Prim 算法
<稠密图prim-算法>
邻接矩阵，$cal(O) (N^2 + M)$。思想同 Dijkstra：`d` 维护每个点到「已在树内点集」的最短边。不连通返回 INF。

#include-code("code/图论/稠密图Prim-算法.cpp")

=== （稀疏图）Kruskal 算法
<稀疏图kruskal-算法>
边按权排序后依次加入，并查集判是否成环，加入 $n - 1$ 条即成树。$cal(O) (M log M)$，瓶颈在排序。不连通则边数不够。
...
从每个左点找增广路：未匹配边前进、已匹配边后退，找到未匹配右点则整条路反转。失败则匹配数无法再加。`match[右点]=左点`。下标 $1 . . n_1$、$1 . . n_2$。复杂度 $cal(O) (N M)$。

#include-code("code/图论/匈牙利算法解.cpp")

=== HopcroftKarp 算法（基于最大流）解
<hopcroftkarp-算法基于最大流解>
该算法基于最大流，常数极小，且引入随机化，几乎卡不掉。最坏时间复杂度为 $cal(O) (sqrt(N) M)$ ，经#link("https://judge.yosupo.jp/problem/bipartitematching")[测试];，在 $N , M$ 均为 $2 times 10^5$ 的情况下能在 60ms 内跑完。
...
通过#strong[递归];的方式来解决#strong[无向正权图];上的全局最小割问题，算法复杂度 $cal(O) (V E + V^2 log V)$ ，一般可近似看作 $cal(O) (V^3)$ 。
...
=== 有向图欧拉路径求解（字典序最小）
<有向图欧拉路径求解字典序最小>
要求每个节点的出边按目标编号#strong[升序];排列（读入后排序，或直接存进 `multiset`），每次取最小出边即保证字典序最小；起点 `s` 按上文判定选取（欧拉回路可任取非孤立点）。注意下方用 `vector::erase(begin())` 每次开头删除是 $cal(O) (deg)$，总复杂度退化为 $cal(O) (M^2)$，边多时改用 `multiset` 或指针法。
...
求任意一组解。做法：把 $x_u - x_v lt.eq w$ 写成 $x_u lt.eq x_v + w$，从 $v$ 向 $u$ 连一条权值为 $w$ 的边跑最短路，最短路数组本身即为一组解；存在负环则无解。下方代码实际是 Bellman–Ford（每轮松弛全部 $m$ 条边、共 $n - 1$ 轮，复杂度 $cal(O) (n m)$，原文标注 SPFA 系误称），存边时 `e[i] = {v, u, w}` 正好实现 $v arrow.r u$ 这条边。#strong[注意这里以 `d[1] = 0` 作单源，图不连通或存在从 1 不可达的点时需改为超级源点：向所有点连权 0 的边（即初始化全部 `d[i] = 0`）。];#link("https://www.luogu.com.cn/problem/P5960")[参考]
...
+ 在竞赛图中，将所有点按入度从小到大排序，随后依次遍历，若对于某一点 $i$ 满足前 $i$ 个点的入度之和恰好等于 $⌊frac(n dot.op (n + 1), 2)⌋$ ，那么对于上一次满足这一条件的点 $p$ ，$p + 1$ 到 $i$ 点构成一个新的强连通分量 #link("https://codeforces.com/contest/1498/problem/E")[See] 。
...
设 $s_i$ 表示每个连通块的数量，通项公式为 $n^(k - 2) dot.op product_(i = 1)^k s_i$ ，当 $k < 2$ 时答案为 $1$ 。
...
=== 本质不同简单环计数
<本质不同简单环计数>
原题：给出一张无向图，输出简单环的数量 #link("https://codeforces.com/contest/11/problem/D")[See] 。注意这里环套环需要分别多次统计，下图答案应当为 $7$。使用状压 dp，复杂度为 $cal(O) (M dot.op 2^N)$，可以扩展到有向图。
