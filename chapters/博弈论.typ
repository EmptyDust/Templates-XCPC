...
- $N = K dot.op (M+1)$（其中 $K in NN^+$），后手必胜（后手可以控制每一回合结束时双方恰好取走 $M+1$ 个，重复 $K$ 轮后即胜利）；
- $N = K dot.op (M+1)+R$（其中 $K in NN^+, 0 < R < M+1$），先手必胜（先手先取走 $R$ 个，之后控制每一回合结束时双方恰好取走 $M+1$ 个，重复 $K$ 轮后即胜利）。
...
- $N = K dot.op (a+b)$ 时，后手必胜；
- $N = K dot.op (a+b)+R_1$（其中 $K in NN^+, 0 < R_1 < a$）时，后手必胜（这些数量不够再取一次，先手无法逆转局面）；
- $N = K dot.op (a+b)+R_2$（其中 $K in NN^+, a <= R_2 <= b$）时，先手必胜；
- $N = K dot.op (a+b)+R_3$（其中 $K in NN^+, b < R_3 < a+b$）时，先手必胜（这些数量不够再取一次，后手无法逆转局面）；
...
*存在某一位，* $bold(O n e_i)$ *不是* $bold(K+1)$ *的倍数。*（原来写成“每一位均不为倍数”，过严，应是存在一位。）
...
有向图上的后继取 $upright("mex")$。下面板子是取石子：先读 $m$ 种可取数量 `a[1..m]`，再读 $n$ 堆石子，每堆求 SG 后异或。`num[]` 记忆化须先置 $-1$（已在 `Solve` 里 `memset`，多测每组都要清）。`yes` / `no` 为赛场宏，须自行 `#define`。`N` 按最大石子数改。复杂度与后继数有关，取石子情形约 #O($sum x dot.op m$)。
...
每回合必须移动所有还能动的棋子；胜负只看各子游戏最长步数——存在奇数 $s t e p$ 则先手必胜。
...
定义 $s t e p$ 为某一局游戏至多需要经过的回合数。

以下局面先手必胜：$s t e p$ 为奇数。
...
更一般地，对于第 $bold(k)$ 对数，第一个数为 $bold(F i r s t_k = lr(floor.l frac(k (1 + sqrt(5)), 2) floor.r))$，第二个数为 $bold(S e c o n d_k = F i r s t_k + k)$。
...
先手不能一次取完，之后每次不超过对方上次的两倍。当且仅当 $N$ 为斐波那契数时先手必败。`Force()` 预处理到 $upright("fib")[86] approx 8.9 times 10^17$，覆盖 `long long`；须在 `Solve` 前调用一次。`n` 超过 `int` 时把读入也改成 `long long`。
