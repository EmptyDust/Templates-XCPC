...
有向图上的后继取 $upright("mex")$。下面板子是取石子：先读 $m$ 种可取数量 `a[1..m]`，再读 $n$ 堆石子，每堆求 SG 后异或。`num[]` 记忆化须先置 $-1$（已在 `Solve` 里 `memset`，多测每组都要清）。`yes` / `no` 为赛场宏，须自行 `#define`。`N` 按最大石子数改。复杂度与后继数有关，取石子情形约 #O($sum x dot m$)。
