...
当放入一个价值为 $w [i]$ 的物品后，价值增加了 $v [i]$，于是我们可以构建一个二维的 $d p [i] [j]$ 数组，装入第 $i$ 件物品时，背包容量为 $j$ 能实现的 #strong[最大价值];，可以得到 #strong[转移方程] $d p [i] [j] = m a x (d p [i - 1] [j] , d p [i - 1] [j - w [i]] + v [i])$。
...
和 01 背包 类似地压缩成一维——注意完全背包是#strong[顺序];遍历（`dp[j-w[i]]` 用的是当轮新值，等价于可以重复取），与 01 背包的逆序相对。
...
尽管采用了 #strong[二进制优化];，时间复杂度还是太高，采用 #strong[单调队列优化];，将时间复杂度优化至 $O (n \* m)$：按 $j med mod med w$ 分组后在每组内做滑动窗口，窗口长度为 $s + 1$，维护 $g [k] - ⌊ (k - j) \/ w ⌋ v$ 的最大值。
...
开一个储存方案数的数组 $c n t$，$c n t [i]$ 表示容量为 $i$ 时的 #strong[方案数];，先将 $c n t$ 的每一个值都初始化为 1，因为 #strong[不装任何东西就是一种方案];，如果装入这件物品使总的价值 #strong[更大];，那么装入后的方案数 #strong[等于] 装之前的方案数，如果装入后总价值 #strong[相等];，那么方案数就是 #strong[二者之和]
...
下方第一个板子统计 $[0 , x]$ 中数字 $d$ 出现的次数（`solve(r, d) - solve(l - 1, d)` 即区间计数）。第二个板子统计 $lt.eq n$ 的数中”数位和能整除该数”的个数：枚举可能的数位和 `mod`（$lt.eq 9 dot.op l e n$），每趟记录 `(余数, 当前数位和)` 两维。
...
== 状压 DP
<状压-dp>
#strong[题意：];在 $n \* n$ 的棋盘里面放 $k$ 个国王，使他们互不攻击，共有多少种摆放方案。国王能攻击到它上下左右，以及左上左下右上右下八个方向上附近的各一个格子，共 8 个格子。做法：预处理出所有”同行不相邻”的合法状态 `st`，转移时要求本行状态与上一行状态（含左右移）按位与为 $0$；复杂度 $cal(O) (n dot.op k dot.op lr(|S|)^2)$，$lr(|S|)$ 为合法状态数（约 $1.6^n$ 量级）。
...
=== 最短 Hamilton 路径
<最短-hamilton-路径>
求从 $0$ 号点出发、恰好经过每个点一次到达 $n - 1$ 的最短路。`f[S][j]` 为经过点集 $S$、终点为 $j$ 的最短路（用位表示集合，`f[1][0] = 0`），枚举上一站 $k$ 转移，复杂度 $cal(O) (N^2 dot.op 2^N)$。
...
== SOSdp 高维前缀和
<sosdp-高维前缀和>
高维前缀和（SOS DP）：对每个二进制位做一维前缀和，$cal(O) (n dot.op 2^n)$。第一段求 `f[i] = Σ f[所有 i 的子集]`（子集向超集贡献），第二段求 `f[i] = Σ f[所有包含 i 的超集]`（超集向子集贡献，注意逆序枚举与补分号）。常用于按”包含关系”计数、子集卷积前置。
...
== 汉明权重
<汉明权重>
Gosper’s hack：按升序枚举 $[0 , n]$ 中#strong[二进制恰好含 $i$ 个 $1$] 的所有数，$cal(O) (upright(组 合 数))$。外层 $i$ 从 $0$ 到 $log_2 (n + 1)$，内层从最小组合 `(1<<i)-1` 通过 `t = x + (x & -x)` 的组合位技巧得到下一个同权重数，常用于枚举大小为 $i$ 的子集状态。
