...
== 汉明权重
<汉明权重>
Gosper’s hack：按升序枚举 $[0 , n]$ 中#strong[二进制恰好含 $i$ 个 $1$] 的所有数，$cal(O)(upright(组 合 数))$。外层 $i$ 从 $0$ 到 $"log"_2 (n + 1)$，内层从最小组合 `(1<<i)-1` 通过 `t = x + (x & -x)` 的组合位技巧得到下一个同权重数，常用于枚举大小为 $i$ 的子集状态。
