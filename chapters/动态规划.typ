...
== 状压 DP
<状压-dp>
#strong[题意：];在 $n \* n$ 的棋盘里面放 $k$ 个国王，使他们互不攻击，共有多少种摆放方案。国王能攻击到它上下左右，以及左上左下右上右下八个方向上附近的各一个格子，共 8 个格子。做法：预处理出所有”同行不相邻”的合法状态 `st`，转移时要求本行状态与上一行状态（含左右移）按位与为 $0$；复杂度 $cal(O)(n dot k dot lr(|S|)^2)$，$lr(|S|)$ 为合法状态数（约 $1.6^n$ 量级）。
...
=== 最短 Hamilton 路径
<最短-hamilton-路径>
求从 $0$ 号点出发、恰好经过每个点一次到达 $n - 1$ 的最短路。`f[S][j]` 为经过点集 $S$、终点为 $j$ 的最短路（用位表示集合，`f[1][0] = 0`），枚举上一站 $k$ 转移，复杂度 $cal(O)(N^2 dot 2^N)$。
...
== SOSdp 高维前缀和
<sosdp-高维前缀和>
高维前缀和（SOS DP）：对每个二进制位做一维前缀和，$cal(O)(n dot 2^n)$。第一段求 `f[i] = Σ f[所有 i 的子集]`（子集向超集贡献），第二段求 `f[i] = Σ f[所有包含 i 的超集]`（超集向子集贡献，注意逆序枚举与补分号）。常用于按”包含关系”计数、子集卷积前置。
