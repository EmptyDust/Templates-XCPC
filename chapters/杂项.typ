...
#pitfall[`lower_bound` 对应#strong[严格];递增、`upper_bound` 对应#strong[非严格];递增——两个 bound 换错，LIS 语义就变了。]
...
#quote(block: true)[
Dilworth: 对于任意有限偏序集，其最大反链中元素的数目必等于最小链划分中链的数目. 将一个序列剖成若干个单调不升子序列的最小个数等于该序列最长严格上升子序列的长度
]
...
=== 二维+输出方案
<二维输出方案>
先按第一维升序，第一维相同则第二维降序（避免等 $x$ 误接），再对第二维做严格 LIS；坐标可为任意 int，包含零和负数。`pre` 记前驱，从最长结尾回溯。
