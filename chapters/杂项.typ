...
== int128 输入输出流控制
<int128-输入输出流控制>
int128 只在 Linux 下可用。约 $38$ 位十进制。下面 `>>` #strong[没有处理负号];（原文写支持负数），负数要自己加。`<<` 也不写负号。
...
== 对拍板子
<对拍板子>
同目录多题：`A.cpp` / `A-std.cpp` / `A-gen.cpp`。 一题多份源（`F.cpp` / `F-1.cpp` / `F-Brute.cpp`）共用该题一份暴力和生成器。 对拍器做成函数，追加到 `~/.bashrc` / `$PROFILE`。 prefix：`stress A` 或 `stress F-1`。被测用完整参数，std/gen 只取第一段连字符前的题号。 manual：`stress a.cpp brute.cpp gen.cpp`，三个路径原样用。 其它个数直接返回。 编译产物与中间文件均在临时目录固定路径，下次覆盖，不 `rm`。 挂了去看临时目录里的输入输出。三个 `g++` 都成功才进入循环。
