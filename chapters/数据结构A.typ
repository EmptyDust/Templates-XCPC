...
= 数据结构 A
<数据结构-a>
区间询问的军火库：并查集、ST 表、树状数组、线段树家族（zkw / 可持久化 / 树套树）、莫队家族、KD 树。选型信号：静态且可重复贡献 → ST 表；单点加 + 区间和 → 树状数组；区间修改或复杂信息 → 线段树；离线区间询问且 `add`/`del` 好写 → 莫队；询问带修改时间 → 带修改莫队。
...
== ST 表
<st-表>
#specline([预处理 #O($N "log" N$)], [查询 #O($1$)])
用于解决区间可重复贡献问题，需要满足 $x upright(" 运算符 ") x = x$ （如区间最大值：$"max" (x , x) = x$ 、区间 $"gcd"$：$"gcd" (x , x) = x$ 等），但是不支持修改操作。下方 `vt` 的第二维硬编码为 `30`（支持 $n lt.eq 2^30$），按 $n$ 改为 `__lg(n)+1` 即可；合并运算由 `Info::operator+` 表达。

#include-code("code/数据结构A/ST-表.cpp")

== Fenwick Tree 树状数组
<fenwick-tree-树状数组>
#specline([单点加 / 前缀和 #O($"log" N$)])
下标从 $1$ 开始。`x & -x` 取出最低位 $1$，沿这条链走到父区间；单点加往上走、前缀和往下走，是同一棵隐式树的对偶。`ask(l, r)` 为 $[l , r]$ 区间和。不能直接区间覆盖（改差分）。构造时从 `in`（下标 $1 dots.c n$）逐点加入。
...
树状数组上按 $2^k$ 步长倍增下落，用前缀和计数定位第 $k$ 小，排名与前驱后继同理。

#pitfall[被查询的值都必须 $lt.eq N$，否则越界；离散化不可用时改用平衡树。]
...
== 二维树状数组
<二维树状数组>
#specline([空间 #O($N M$)], [建树 #O($N M$)], [单查 #O($"log" N dot "log" M$)])

#pitfall[封装一不能同时进行区间修改 + 区间查询；都要用请换封装二。]
...
需要额外维护一个区间最大值，当模数大于区间最大值时剪枝，否则进行单点取模。由于单点 $upright("MOD") < x$ 时 $x mod upright("MOD") < x / 2$ ，故单点取模至 $0$ 最劣只需要 $"log" x$ 次 。
...
== 树套树
<树套树>
#specline([单次 #O($"log"^2 N$)])
外层按位置分区间，内层按值排序：一次操作拆成 $"log" N$ 个外节点，每个再 $"log" N$。用来做动态区间第 $k$ 小、排名、前驱后继。
...
== 小波矩阵树：高效静态区间第 K 大查询
<小波矩阵树高效静态区间第-k-大查询>
#specline([建树 #O($N "log" N$)], [空间 #O($N + frac(N "log" N, 64)$)], [单查 #O($"log" N$)])
手写 `bitset` 压位。建议使用 $mono("0-idx")$ 计数法，但是经测试 $mono("1-idx")$ 也有效，但需要更多的检验。使用前定义 `using u64 = unsigned long long;`。

#include-code("code/数据结构A/小波矩阵树：高效静态区间第-K-大查询.cpp")

== 主席树 \(可持久化线段树)
<主席树-可持久化线段树>
#specline([建树/单点修改 #O($N "log" N$)], [查询 #O($"log" N$)])
原理：每次修改只新建根到目标叶子的一条链（$"log"$ 个节点），其余子树与旧版本共享——`root[i]` 因此就是「前 $i$ 个元素」的权值线段树，区间 $[l , r]$ 的信息由两版本相减得到（`kth` 里的 `cnt` 相减即此）。用法：把值域离散化到 $[1 , m]$，`modify(root[i] = root[i - 1], ..., 离散值)` 建版本；静态区间第 $k$ 小查询 `kth(root[r], root[l - 1], 1, m, k)`。数组开 `4 * N + 17 * N`（约 17 层/次修改），`N` 按题目改。

#include-code("code/数据结构A/主席树-可持久化线段树.cpp")

== 普通莫队
<普通莫队>
#specline([区间移动均摊 #O($sqrt(N)$)])
离线区间询问：按左端点所在块排序，同一块内右端点来回扫（奇偶块反向），相邻询问的区间差均摊 $O (sqrt(N))$。先写 `add`/`del` 维护当前窗口答案 `val`，再扩缩 $[l , r]$。下标 $1 dots.c n$；块长可取 $n \/ sqrt(q)$，也可 $sqrt(n)$ 或 $317$（对应 $n = 10^5$）。
...
<带修改的莫队-带时间维度的莫队>
#specline([均摊 #O($n^(5 \/ 3)$)])
普通莫队再加时间维：询问记下「做到第几次修改」，$l , r , t$ 三个指针一起移动。块长取 $n^(2 \/ 3)$（$n = 10^5$ 时约 $2154$，也可用 `pow(n, 2./3)`）。`time` 把修改点换进窗口或撤销；`a2`/`a4` 需自行定义，`add`/`del` 为钩子，答案记在 `val`。
...
== 回滚莫队
<回滚莫队>
#specline([均摊 #O($N sqrt(Q) dot T_(a d d)$)])
用于#strong[删除难实现];的问题（信息只有“加”容易撤销：如并查集、众数）：只比普通莫队多一个“右指针单调向右，左指针临时左移后回滚”的技巧。同一块内的询问直接暴力（三次遍历做“加入/统计/撤销”），跨块询问中右端点只增、左端点每次临时扩展后 `del` 撤销（#strong[不真正删除];，而是退回临时指针）。`a3 = array<int, 3>` 需自行定义，`add(x, res)` 中 `res` 引用传递更新答案。
...
== KD Tree
<kd-tree>
#specline([单次最坏 #O($n^(1 - k^(- 1))$)（$k$ 为维数）])
数组版（容量 `N = 1e5 + 10`，`K` 维），`insert` 平衡因子 $alpha = 0.725$ 触发拍平重建；`build` 按#strong[方差最大的维];划分、`nth_element` 取中位数。下方 `query(a)` 统计#strong[所有维都 ≤ a\[i\]] 的点数（`out` 剪枝整块在查询域之外，`all` 整块皆计入，`in` 单点判定）；常见扩展：最近点查询改成带界最值剪枝。
