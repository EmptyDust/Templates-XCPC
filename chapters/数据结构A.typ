...
== 二维树状数组
<二维树状数组>
#strong[封装一：该版本不能同时进行区间修改+区间查询。];无离散化版本的空间占用为 $cal(O)(N M)$ 、建树复杂度为 $cal(O)(N M)$ 、单次查询复杂度为 $cal(O)("log" N dot "log" M)$ 。
...
== 回滚莫队
<回滚莫队>
用于#strong[删除难实现];的问题（信息只有”加”容易撤销：如并查集、众数）：只比普通莫队多一个”右指针单调向右，左指针临时左移后回滚”的技巧。同一块内的询问直接暴力（三次遍历做”加入/统计/撤销”），跨块询问中右端点只增、左端点每次临时扩展后 `del` 撤销（#strong[不真正删除];，而是退回临时指针）。复杂度 $cal(O)(N sqrt(Q) dot T_(a d d))$。`a3 = array<int, 3>` 需自行定义，`add(x, res)` 中 `res` 引用传递更新答案。
